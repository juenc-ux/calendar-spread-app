...
export default function ForwardVolCalculator() {
  ...
  const fetchOptionData = async (tickerSymbol) => {
    ...
    try {
      ...
      try {
        const chainUrl = `https://api.polygon.io/v3/snapshot/options/${symbol}?contract_type=call&include_greeks=true&limit=1000&order=asc&sort=expiration_date&apiKey=${apiKey}`;
        ...
        console.log('Options chain data:', chainData);

        if (chainData.status === 'ERROR') {
          throw new Error(chainData.error || 'Options snapshot error');
        }

        const rawResults = Array.isArray(chainData.results) ? chainData.results : [];

        const callOptions = rawResults.filter(opt => {
          const type = (opt.details?.contract_type || opt.contract_type || opt.option_type || '').toLowerCase();
          return type === 'call';
        });
        ...
        const parseOptionData = (option, snapshot = null) => {
          if (!option && !snapshot) return null;

          const source = snapshot || option || {};
          const detailSource = snapshot?.details || option?.details || {};

          const expiration = detailSource.expiration_date || option?.expiration_date || snapshot?.expiration_date || null;
          const strike = detailSource.strike_price ?? option?.strike_price ?? snapshot?.strike_price ?? null;
          const tickerSymbol = detailSource.ticker || option?.ticker || option?.option_symbol || snapshot?.ticker || snapshot?.option_symbol || null;

          const ivRaw = source.implied_volatility
            ?? source.greeks?.mid_iv
            ?? source.greeks?.mark_iv
            ?? source.greeks?.iv
            ?? source.last_quote?.implied_volatility
            ?? null;

          const volumeRaw = source.day?.volume ?? source.last_quote?.volume ?? source.volume ?? null;
          const openInterestRaw = source.open_interest ?? source.day?.open_interest ?? null;
          ...
          return {
            ...
            volume: volumeRaw != null ? Number(volumeRaw) : null,
            openInterest: openInterestRaw != null ? Number(openInterestRaw) : null,
          };
        };

        const fetchContractSnapshot = async (option) => {
          if (!option) return null;

          const tickerSymbol = option.details?.ticker || option.ticker || option.option_symbol;
          if (!tickerSymbol) {
            return parseOptionData(option);
          }

          const contractUrl = `https://api.polygon.io/v3/snapshot/options/${tickerSymbol}?include_greeks=true&apiKey=${apiKey}`;

          try {
            const contractResponse = await fetch(contractUrl);
            if (!contractResponse.ok) {
              throw new Error(`Contract snapshot error (${contractResponse.status})`);
            }

            const contractData = await contractResponse.json();
            const snapshotPayload = contractData.results ?? contractData.result ?? null;

            if (!snapshotPayload) {
              throw new Error('Empty contract snapshot payload');
            }

            return parseOptionData(option, snapshotPayload);
          } catch (snapshotError) {
            console.warn('Failed to hydrate option snapshot, using chain data only:', snapshotError.message);
            return parseOptionData(option);
          }
        };
        ...
        const [exp1Data, exp2Data] = await Promise.all([
          fetchContractSnapshot(option1),
          fetchContractSnapshot(option2)
        ]);
